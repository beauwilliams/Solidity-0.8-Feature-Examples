...
contract SafeMath {
    function testUnderflow() public pure returns (uint) {
        uint x = 0;
        //NEW: Without safemath library, we would have to check for underflow
        x--;
        return x;
    }
    ...
    function testUncheckedUnderflow() public pure returns (uint) {
        uint x = 0;
        // We can still overflow if we want to
        unchecked { x--; }
        return x;
    }
}
...
contract VendingMachine {
    ...
    function withdraw() public {
        if (msg.sender != owner)
            //NEW: Here we can use our custom error - which also saves gas
            revert Unauthorized();
